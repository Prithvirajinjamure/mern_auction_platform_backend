...
const AuctionItem = ({ auctionItem }) => {
    ...
    }, [isAuthenticated, navigateTo, dispatch, id]);
                        ...
                        <div className="w-full lg:w-[400px] flex flex-col bg-white rounded-lg shadow-md min-h-[400px]">
                                    ...
                                    <>
                                        {new Date(auctionDetail.startTime) > Date.now() ? (
                                            ...
                                            [...auctionBidders]
                                                ...
                                                .map((element, index) => (
                                                    <div key={index} className="py-2 flex items-center justify-between border-b border-gray-100">
                                                        ...
                                                        {index === 0 ? (
                                                            <span className="text-green-600 font-bold">1st</span>
                                                        ) : index === 1 ? (
                                                            <span className="text-blue-600 font-bold">2nd</span>
                                                        ) : index === 2 ? (
                                                            <span className="text-yellow-600 font-bold">3rd</span>
                                                        ) : (
                                                            <span className="text-gray-600 font-bold">{index + 1}th</span>
                                                        )}
                                                    </div>
                                                ))
                                        ) : (
                            ...
                            {auctionDetail && Date.now() >= new Date(auctionDetail.startTime) && 
                             Date.now() <= new Date(auctionDetail.endTime) && (
                                <div className="bg-[#D6482B] py-4 px-6 flex items-center justify-between rounded-b-lg">
                                    <div className="flex gap-3 flex-col sm:flex-row sm:items-center">
                                        <span className="text-[30px] text-white font-semibold">Place Bid</span>
                                        <input
                                            type="number"
                                            className="w-24 focus:outline-none text-[16px] md:text-[20px] p-1 rounded"
                                            value={amount}
                                            onChange={(e) => setAmount(e.target.value)}
                                        />
                                    </div>
                                    <button
                                        className="p-3 text-white bg-black text-[30px] rounded-full transition-all duration-300 hover:bg-[#222]"
                                        onClick={handleBid}
                                    >
                                        <RiAuctionFill />
                                    </button>
