    ...
    for (const proof of approvedProofs) {
      try {
        ...
        if (user) {
          ...
          // Fix the typo in email parameter and add error handling
          try {
            await sendEmail({
              email: user.email,  // Fixed typo from 'emamil' to 'email'
              subject,
              message
            });
        ...
        console.log(
          ` user ${proof.userId} paid commmission of ${proof.amount} processed successfully.`
        ); 
